...
class _TasksDisplay extends State<TasksDisplay> {
  ...
  Future _pickDateTime({required initialDate, required int who}) async {
  ...
  void _segregateComplete() {
    setState(() {
      _segregatedTasks =
          widget._tasksToDisplay.where((task) => task.isDone).toList();
      _isSegregated = true;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    (_isSegregated ? _segregatedTasks : widget._tasksToDisplay).sort(
      (task1, task2) => sortTasks(task1, task2),
    );
